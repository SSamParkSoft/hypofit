...
import com.contentruck.hypofit.block.application.UserBlockCommand;
...
final class UserBlockCreateRequestParser {

    private UserBlockCreateRequestParser() {
    }

    static UserBlockCommand parse(UserBlockCreateRequest request) {
        return parse(RawRequestBodyJson.toJsonNode(request.rawBody()));
    }

    static UserBlockCommand parse(JsonNode body) {
