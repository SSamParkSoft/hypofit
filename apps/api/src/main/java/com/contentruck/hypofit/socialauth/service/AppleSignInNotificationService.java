...
public class AppleSignInNotificationService {
    ...
    static final long UNKNOWN_KEY_REFRESH_COOLDOWN_SECONDS = 30L;
    ...
    private volatile long unknownKeyRefreshAllowedAtEpochSecond = 0L;
    ...
    private JWK getJwk(String keyId) {
        ...
        JWK cachedKey = keys.get(keyId);
        if (cachedKey != null && now < jwksCacheExpiresAtEpochSecond) {
            return cachedKey;
        }
        if (cachedKey == null && now < unknownKeyRefreshAllowedAtEpochSecond) {
        ...
        try {
            ...
            keys = cachedKeysById;
            cachedKey = keys.get(keyId);
            if (cachedKey != null && refreshedNow < jwksCacheExpiresAtEpochSecond) {
                return cachedKey;
            }
            if (cachedKey == null && refreshedNow < unknownKeyRefreshAllowedAtEpochSecond) {
                throw invalidNotification("Unknown Apple notification key");
            }

            Map<String, JWK> refreshedKeys = Map.copyOf(jwksClient.fetchKeys());
            cachedKeysById = refreshedKeys;
            jwksCacheExpiresAtEpochSecond = refreshedNow + jwksCacheSeconds();
            cachedKey = refreshedKeys.get(keyId);
            if (cachedKey == null) {
                unknownKeyRefreshAllowedAtEpochSecond = refreshedNow + UNKNOWN_KEY_REFRESH_COOLDOWN_SECONDS;
                throw invalidNotification("Unknown Apple notification key");
            }
            unknownKeyRefreshAllowedAtEpochSecond = 0L;
            return cachedKey;
        } finally {
