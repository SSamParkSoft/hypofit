...
import { getCounterpart } from "../model/chatRoomModel";
    ...
    <Dialog.Root open onOpenChange={(isOpen) => !isOpen && onClose()}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed bottom-[calc(var(--app-safe-bottom)+0.75rem)] left-1/2 z-50 grid max-h-[calc(100dvh-var(--app-safe-top)-var(--app-safe-bottom)-3rem)] w-[calc(100%-1.5rem)] max-w-md -translate-x-1/2 gap-4 overflow-y-auto rounded-hypo-lg border border-hypo-border bg-hypo-surface p-4 focus:outline-none md:bottom-auto md:top-1/2 md:-translate-y-1/2">
          ...
          <p className="rounded-hypo-lg border border-hypo-border bg-hypo-bg px-3 py-2.5 text-sm font-semibold leading-6 text-hypo-text">
            {counterpart.bio ?? "아직 한줄소개가 없습니다."}
          </p>

          <div className="rounded-hypo-lg border border-hypo-border bg-hypo-bg px-3 py-3">
