...
export function ProfileIdentityPreview({
  ...
  const organizationName = appUser?.organization_name?.trim() || null;
      ...
      <div className="flex flex-wrap items-end justify-between gap-2 px-1">
        <div>
          ...
          <p className="mt-1 text-xs text-hypo-text-soft">
            사진, 이름{organizationLabel ? ", 소속" : ""}과 한줄소개만 보여요.
          </p>
        </div>
        <span className="rounded-hypo-pill border border-hypo-brand/15 bg-hypo-brand-soft px-2.5 py-1 text-[11px] font-bold text-hypo-brand">
          미리보기
        </span>
      </div>

      <div className="mt-3 overflow-hidden rounded-hypo-lg border border-hypo-brand/15 bg-hypo-brand-soft/50">
        <div className="flex min-h-[132px] items-center gap-4 px-5 py-5 sm:gap-5 sm:px-6">
          ...
          <div className="min-w-0 flex-1">
            ...
            {organizationLabel ? (
              <p className="mt-1 truncate text-sm font-semibold text-hypo-text-muted">
                {organizationLabel}
              </p>
            ) : null}
            <p className="mt-2 line-clamp-2 max-w-xl text-sm leading-6 text-hypo-text-muted">
              {bio}
            </p>
          </div>
        </div>
